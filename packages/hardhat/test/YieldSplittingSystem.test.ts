...
import { YieldSplitter, WrappedU2U, PrincipalToken, YieldToken } from "../typechain-types";
...
describe("Yield Splitting System", function () {
  ...
  beforeEach(async function () {
    ...
    // Wrap some U2U for testing
    await wrappedU2U.connect(user1).deposit({ value: DEPOSIT_AMOUNT });
    await wrappedU2U.connect(user2).deposit({ value: DEPOSIT_AMOUNT });
    
    // Approve YieldSplitter to spend wU2U
    await wrappedU2U.connect(user1).approve(await yieldSplitter.getAddress(), DEPOSIT_AMOUNT);
    await wrappedU2U.connect(user2).approve(await yieldSplitter.getAddress(), DEPOSIT_AMOUNT);
  });

  describe("Deployment", function () {
    it("Should deploy all contracts correctly", async function () {
      expect(await wrappedU2U.name()).to.equal("Wrapped U2U");
      expect(await principalToken.name()).to.equal("Principal Token wU2U");
      expect(await yieldToken.name()).to.equal("Yield Token wU2U");
    });
  ...
  describe("Deposit and Split", function () {
    it("Should deposit and split wU2U into PT and YT", async function () {
      ...
      expect(await wrappedU2U.balanceOf(await yieldSplitter.getAddress())).to.equal(splitAmount);
    });
  ...
  describe("Yield Generation and Claiming", function () {
    ...
    it("Should allow yield claiming", async function () {
      ...
      const initialBalance = await wrappedU2U.balanceOf(user1.address);
      
      // Fund the contract with some U2U for yield distribution
      ...
      const finalBalance = await wrappedU2U.balanceOf(user1.address);
  ...
  describe("Redemption Before Maturity", function () {
    ...
    it("Should allow redemption of PT+YT for wU2U before maturity", async function () {
      ...
      expect(await wrappedU2U.balanceOf(user1.address)).to.equal(initialBalance + redeemAmount);
    });
