...
const OROCHI_ORACLE_ADDRESS = '0x...' as const
const MOCK_AMM_ADDRESS = '0x...' as const

const OROCHI_ORACLE_ABI = [
  {
    name: 'getLatestPrice',
    type: 'function',
...
export function PriceChart() {
  ...
  const { data: u2uPrice } = useReadContract({
  ...
  useEffect(() => {
    const generateMockHistory = () => {
      ...
      for (let i = intervals; i >= 0; i--) {
        ...
        const baseU2Uprice = 0.5
        const basePtPrice = 0.48
        const baseYtPrice = 0.02

        const volatility = 0.02 // 2% volatility
        const u2uPricePoint = baseU2Uprice * (1 + (Math.random() - 0.5) * volatility)
  ...
  const currentU2UPrice = u2uPrice ? parseFloat(formatEther(u2uPrice)) : 0.5
      ...
      <div className='card-body'>
        ...
        <div className='grid grid-cols-3 gap-2 mb-4'>
          <div className='stat bg-accent/10 rounded p-2'>
            <div className='stat-title text-xs'>U2U</div>
            <div className='stat-value text-sm'>${formatPrice(currentU2UPrice)}</div>
        ...
        <div className='bg-base-200 rounded-lg p-4 mb-4'>
          ...
          <div className='h-32 flex items-end justify-between gap-1'>
            {priceHistory.slice(-20).map((data, index) => {
              const maxPrice = Math.max(...priceHistory.map((d) => d.u2uPrice))
              const minPrice = Math.min(...priceHistory.map((d) => d.u2uPrice))
              const normalizedHeight = ((data.u2uPrice - minPrice) / (maxPrice - minPrice)) * 100
              ...
              return (
                <div
                  ...
                  data-tip={`$${formatPrice(data.u2uPrice)}`}></div>
              )
            })}
          </div>
          <div className='text-xs opacity-50 mt-1'>U2U Price Trend</div>
        </div>
        ...
        <div className='space-y-2'>
          <div className='flex justify-between items-center'>
            ...
            <span className='font-semibold'>{ptytRatio ? parseFloat(formatEther(ptytRatio)).toFixed(2) : '24.00'}</span>
          </div>
          ...
          {poolInfo && (
            <>
              ...
              <div className='flex justify-between items-center'>
                ...
                <span className='font-semibold'>{parseFloat(formatEther(poolInfo[2])).toFixed(4)} YT</span>
              </div>
            </>
          )}
        </div>
        ...
        <div className='alert alert-info mt-4'>
          ...
          <div className='text-xs'>
            ...
            <div>PT trading at {((currentPtPrice / currentU2UPrice - 1) * 100).toFixed(1)}% discount to underlying</div>
          </div>
        </div>
      </div>
    </div>
